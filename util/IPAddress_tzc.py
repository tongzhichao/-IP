...
#该函数通过查询纯真IP文件来获取IP对应的国家及区域，表示没看懂，涉及到字节偏移
class IPAddresss:
	...
	def str2ip(self,ippdr):#IP转成二进制字节,估计作者为了兼容python2
		#a=socket.inet_aton(s) ==>  b'\xc0\xa8\x01\x01'
		#struct.unpack(a) ==>(16885952,)
		(ip,) = struct.unpack('I',socket.inet_aton(ippdr))
		return ((ip>>24) & 0xff) | ((ip&0xff)<<24)|((ip>>8)&0xff00)|((ip&0xff00)<<8)




	def ip2str_tzc(self,ip):

		return struct.unpack('!I',socket.inet_aton(ip))[0]

	def str2ip_tzc(self,ip):
